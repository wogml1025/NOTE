...
<<<<<<< HEAD
  ...
  var data2={
    'folderimage' : 'setImage'
  }
  $.ajax({
    type:'POST',
    url:'/mynote/setfolderimage',
    contentType:'application/x-www-form-urlencoded; charset=UTF-8',
    cache:false,
    dataType:'json',
    data:data2,
    success:function(result){
      if(result['result'] == 'success'){
        var folder = result['folderimg'];
        for(var i=0;i<folder.length;i++){
          var f = folder[i].foldername;
          var fi = folder[i].backimage;
          for(var j=0;j<folderInDB.length;j++){
            if(f == folderInDB[j].innerText){
              folderInDB[j].style.backgroundImage='url('+fi+')';
            }//if
          }//inner for
        }//for
      }//if
