...
window.onload=function(){
  ...
  $.ajax({
    type:'POST',
    url:'/mynote/setsharefolderimage',
    contentType:'application/x-www-form-urlencoded; charset=UTF-8',
    cache:false,
    dataType:'json',
    data:data2,
    success:function(result){
      if(result['result'] == 'success'){
        var folder = result['share'];
        for(var i=0;i<folder.length;i++){
          var sf = folder[i].foldername;
          var sfi = folder[i].backimage;
          for(var j=0;j<shareFolder.length;j++){
            if(sf == shareFolder[j].innerText){
              shareFolder[j].style.backgroundImage='url('+sfi+')';
            }//if
          }//inner for
        }//for
      }
